...
class Traccar {
  ...
  static Future<List<Device>?> getDevices() async {
    final response =
        await http.get(Uri.parse(serverURL! + "/api/devices"), headers: headers);
  ...
  static Future<List<PositionModel>?> getPositionById(
    ...
    final response = await http.get(
        Uri.parse(
            serverURL! + "/api/positions?deviceId=" + deviceId + "&id=" + posId),
        headers: headers);
    if (response.statusCode == 200) {
      Iterable list = json.decode(response.body);
  ...
  static Future<http.Response> updateDevices(String fence, String id) async {
    ...
    /// For more fine-grained control over the request, use [Request] or
    /// [StreamedRequest] instead.
    final response = await http.put(Uri.parse(serverURL! + "/api/devices/" + id),
        body: fence, headers: headers);
    return response;
  }
